...
const browserManager = new browser_manager_1.BrowserManager();
    ...
    try {
        switch (name) {
            ...
            case 'browser_new_page': {
                const result = await browserManager.newPage(args?.name);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'browser_navigate': {
                const result = await browserManager.navigate(args?.pageId, args?.url);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'browser_screenshot': {
                const result = await browserManager.screenshot(args?.pageId, {
            ...
            case 'browser_get_visible_elements': {
                const result = await browserManager.getVisibleElements(args?.pageId);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'browser_get_content': {
                const result = await browserManager.getPageContent(args?.pageId);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'browser_click': {
                const result = await browserManager.click(args?.pageId, args?.selector);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'browser_type': {
                const result = await browserManager.type(args?.pageId, args?.selector, args?.text);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'browser_wait_for': {
                const result = await browserManager.waitForSelector(args?.pageId, args?.selector, args?.timeout);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'browser_evaluate': {
                const result = await browserManager.evaluate(args?.pageId, args?.script);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'browser_close_page': {
                const result = await browserManager.closePage(args?.pageId);
                return {
                    content: [
                        {
                            type: 'text',
